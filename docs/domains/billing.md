...
- Enviar remisión / factura desde la tarjeta del kanban abre `AdministrativeSendAdminDocModal` (`app/components/administrative/SendAdminDocModal.vue`). Pregunta si los folios van solo a este rescate o a otros (`extra_rescues`). El PDF de OC es opcional (`UFileUpload`, schema `oc_pdf` en `app/schemas/rescue-admin-doc.ts`): al enviar se sube a Firebase (`uploadFileToFirebaseGeneral`, carpeta `rescue-2/rescue/:id/oc_pdf`) y el body de `POST /api/rescue/admin_doc/:id/` incluye `oc_pdf` (URL o `null`) junto a `remittance_folio`, `invoice_folio` y `extra_rescues`. Composable: `useRescueAdminDoc`.
...
- En la tabla de detalle, **PDF OC** (`oc_pdf`) y **Factura** (`factura`) van al inicio (después de alerta y comentarios). Son editables 1×1:
  - `oc_pdf`: botón ghost para subir o reemplazar un PDF. Se sube a Firebase (`uploadFileToFirebaseGeneral`, carpeta `rescue-2/rescue/:id/oc_pdf`) y se envía la URL en `POST /api/rescue/admin_doc/:id/` (`useRescueAdminDoc`).
  - `factura`: input compacto (Enter o blur). Body: `pendingInvoiceAdminDocToBody` — `invoice_folio`, `oc_pdf` (el valor actual de la fila si no cambia), `remittance_folio: null`, `extra_rescues: []`. La remisión no aplica en esta pantalla.
  - Tras guardar, `useRescueAdminDoc` invalida la lista y el summary de Por facturar (y el tablero administrativo).
- Mapper: `invoice_folio` (canónico), con alias `invoice_number` y `factura`. Componentes: `PendingInvoiceOcPdfCell`, `PendingInvoiceFacturaCell`. Schema: `app/schemas/pending-invoice.ts`.
