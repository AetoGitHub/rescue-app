...
- Rango de fechas en cabecera (`PendingInvoiceDateRangeFilter`): query params `start_date` y `end_date`. El usuario elige un día de calendario; al API se envía ISO-8601 **con el offset de la zona horaria local** (`getLocalTimeZone()`): inicio del día (`T00:00:00±HH:mm`) y fin del día (`T23:59:59±HH:mm`). Ejemplo: `2026-09-01T00:00:00-06:00`. No se manda `YYYY-MM-DD` ni UTC `Z`. Los mismos params van a lista, summary, `by_responsible` y `company_matrix`.
