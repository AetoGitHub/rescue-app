...
- Por facturar: columnas `oc_pdf`, `purchase_order` (OC) y `factura` al inicio de la tabla de detalle. El botón Subir abre `AdministrativeSendAdminDocModal` (remisión, factura y PDF; sin selector de varios rescates) y envía `POST /api/rescue/admin_doc/:id/`.
